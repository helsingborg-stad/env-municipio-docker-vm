...
The wizard now also asks for a **database root password** in addition to the application database password. In standalone mode both may be generated by pressing Enter; in cluster mode both must be entered, and must be identical on both data VMs.

Choose `swarm` in the wizard to set `DOCKER_SWARM=1`. Standalone initializes a one-node Swarm. In a cluster, bootstrap initializes the preferred VM as manager and the secondary joins as a worker. Swarm manages the application only; MariaDB and Caddy stay per-VM Compose services. See [Swarm mode](components/swarm.md) for the join sequence.
